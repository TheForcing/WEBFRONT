...
const s2=String("Modern JavaScript"); // String객체 이용
console.log(typeof s1, typeof s2);

//Property: .length-문자열의 길이
// 모든 문자열 내부 데이터는 인덱스로 접근가능
console.log(s1, ",length=",s1.lenghth);
...
console.log(s1.substring(7)); // 7번 인덱스부터 ~ end까지
console.log(s1.substring(7,17)); //7번 인덱스부터 ~17번인덱스 앞까지
...
position += "JavaScript".length; //검색시작 위치 변경
position = s1.indexOf("Java", position); // 검새 범위제한
console.log("2nd Java:", position);
position += "Java".length; //검색 시작 위치 변경
position = s1.indexOf("Java".position); //검색어 없음
console.log("No keyword:",position); // 찾는 검색어가 없으면 -1

// 문자열 치환: replace
console.log(s1.replace("JavaScript", "JS"));  //뭄자열 내의 JavaScript->Java
// String 은 불변 자료형: 내부 데이터는 변경되지 않는다.
console.log("원본:",s1);
// 좌우 화이트 스페이스 제거: trim
