function testForEach(){
    //forEach 매서드: 배열의 요소를 하나씩 콜백에 전달
    ...
    source.forEach((item, index, arr)=>{
        ...
        console.log(`배열 ${arr}의 ${index}번쨰 요소 ${item}`);
    })
}
//testForEach();
function testSome() {
    ...
    //some,any: 일부 요소가 콜백 함수의 조건을 만족 시킬떄 ture
...
function testEvery() {
    ...
    //every: 모든 요소가  콜백함수의 조건을 만족 시킬떄 true
    let result = data.every(x=>{
        return x.age >25;
    });


    console.log("-----every: aget>25?", result);
}
